...
from .pwner import Pwner
import re

class Gui(Pwner):
	def __init__(self):
		self.root = tk.Tk()
		self.args = {"printable": False, "binary": None, "libc": None, "mode": "local"}
		self.main()
	...
	def btn_run_exec(self):
		...
		self.root.destroy()
		attack = Pwner({k:v for k,v in self.args.items() if v is not None})
		attack.main()
	...
	def checkbox_printable(self):
		...
		"""
		self.args["printable"] = not self.args["printable"]